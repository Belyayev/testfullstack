...
export const resendConfirmationLink = createAsyncThunk(
  ...
  async (user, thunkAPI) => {
    try {
      return await authService.resendConfirmationLink(user);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const confirmUser = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      return await authService.confirmUser(id);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const passwordResetRequest = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    try {
      return await authService.passwordResetRequest(payload);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const passwordResetExecution = createAsyncThunk(
  "auth/passwordresetexecution",
  async (payload, thunkAPI) => {
    try {
      return await authService.passwordResetExecution(payload);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(passwordResetExecution.rejected, (state, action) => {
        state.isLoading = false;
        state.isError = true;
        state.message = action.payload;
        state.user = null;
